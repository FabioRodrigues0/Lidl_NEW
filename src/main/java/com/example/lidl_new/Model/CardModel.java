...
public class CardModel {
    ...
    public static int UpdatePointsCard(int idClient, double totalPurshase){
        ...
        try {
            ...
            ResultSet rs = stmt2.executeQuery("SELECT client.id, client.card, client_card.points FROM client " +
                    "INNER JOIN client_card on client.card = client_card.id where client.id = '" + idClient + "'");
            while (rs.next()){
                cardId = rs.getInt(2);
                pointsAtMoment = rs.getInt(3);
                System.out.println("Id do cliente: " + rs.getInt(1));
                System.out.println("Id do cartão cliente: " + rs.getInt(2));
                System.out.println("Pontos no cartão cliente: " + rs.getInt(3));
            }
        } catch (Exception e) {
